...
@app.get('/get_apps')
async def get_apps(request: Request, access_code: str):
    ...
    with open('apps.json', 'r') as f:
        ...
        for app in apps:
            ...
            # get version
            for osName in ['win', 'mac', 'linux']:
                folder = os.path.join('static', app['folder'], 'zips', osName)
                if os.path.exists(folder):
                    ...
                    if len(paths) > 0:
                        ...
                        app['version_'+osName] = version
                        app['download_'+osName] = f"{prepend_url}{latest_file}".replace(
                            '\\', '/')
